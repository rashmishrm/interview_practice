...
public class Q787_FindCheapestPrice {

	public static int findCheapestPrice(int n, int[][] flights, int src, int dst, int K) {

		Map<Integer, List<List<Integer>>> adjancyList = new HashMap<>();

		for (int i = 0; i < flights.length; i++) {
			List<List<Integer>> list = adjancyList.getOrDefault(flights[i][0], new ArrayList<>());
			List<Integer> inner = new ArrayList<Integer>();
			inner.add(flights[i][1]);
			inner.add(flights[i][2]);
			list.add(inner);
			adjancyList.put(flights[i][0], list);
		}
		...
		dfs(dst, 0, -1, K, src, adjancyList, prices, traversed);

		return prices.peek() == null ? -1 : prices.poll();

	}

	public static void dfs(int dest, int cost, int level, int k, int current,
			Map<Integer, List<List<Integer>>> adjancyList, PriorityQueue<Integer> prices, Set<Integer> traversed) {
		...
		List<List<Integer>> list = adjancyList.getOrDefault(current, new ArrayList<>());

		level++;
		for (List<Integer> e : list) {
			// cost=cost+e.get(1);
			if (e.get(0) == dest) {
				prices.offer(cost + e.get(1));

			} else {
				Set<Integer> abc=new HashSet<>();
				abc.addAll(traversed);
				
				dfs(dest, cost + e.get(1), level, k, e.get(0), adjancyList, prices,abc);
...
//		int[][] array = { { 0, 12, 28 }, { 5, 6, 39 }, { 8, 6, 59 }, { 13, 15, 7 }, { 13, 12, 38 }, { 10, 12, 35 },
...
//				{ 12, 6, 24 }, { 9, 15, 94 }, { 5, 7, 77 }, { 4, 10, 18 }, { 7, 2, 11 }, { 9, 5, 41 } };
		...
		int[][] array={{3,4,4},{2,5,6},{4,7,10},{9,6,5},{7,4,4},{6,2,10},{6,8,6},{7,9,4},{1,5,4},{1,0,4},{9,7,3},{7,0,5},{6,5,8},{1,7,6},{4,0,9},{5,9,1},{8,7,3},{1,2,6},{4,1,5},{5,2,4},{1,9,1},{7,8,10},{0,4,2},{7,2,8}}
;

		System.out.println(findCheapestPrice(17, array, 6, 0, 7));
	}
