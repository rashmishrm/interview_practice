...
public class subset {
	...
	private void backtracksame(List<List<Integer>> list, List<Integer> tempList, int[] nums, int remain, int start) {
		if (remain < 0)
			...
			for (int i = start; i < nums.length; i++) {
				tempList.add(nums[i]);
				backtracksame(list, tempList, nums, remain - nums[i], i); // not i + 1 because we can reuse same
																			// elements
				tempList.remove(tempList.size() - 1);
			}
		}
	...
	private void backtracksame3(List<List<Integer>> list, List<Integer> tempList, int[] nums, int remain, int start) {
		...
		if (tempList.size() == 3 && remain == 0)
			...
			for (int i = start; i < nums.length; i++) {
				tempList.add(nums[i]);
				backtracksame3(list, tempList, nums, remain - nums[i], i); // not i + 1 because we can reuse same
																			// elements
				tempList.remove(tempList.size() - 1);
			}
		}
	...
	public List<List<Integer>> combinationSumk(int[] nums, int k) {
		List<List<Integer>> list = new ArrayList<>();
		// Arrays.sort(nums);
		backtracksamk(list, new ArrayList<>(), nums, 0, 0, k);
		return list;
	}

	private void backtracksamk(List<List<Integer>> list, List<Integer> tempList, int[] nums, int remain, int start,
			int k) {
		// if(remain < 0) return;
		...
		if (tempList.size() == k && remain == 0)
			...
			for (int i = start; i < nums.length; i++) {
				tempList.add(nums[i]);
				backtracksamk(list, tempList, nums, remain - nums[i], i, k); // not i + 1 because we can reuse same
																				// elements
				tempList.remove(tempList.size() - 1);
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(new subset().combinationSumk(new int[] { -5, 1, 10, 2, 3 },4));
